...
st.markdown("""
<style>
/* Base: todo más grande */
html, body, [class*="css"] {
    font-size: 19px !important;
}

/* Texto general, párrafos, divs */
p, div, span, li, td, th {
...
h2, [data-testid="stHeading"] h2 {
    ...
    border-bottom: 3px solid #c5cae9 !important;
    padding-bottom: 0.4rem !important;
    letter-spacing: -0.5px !important;
}

/* st.subheader() → h3 — apunta directo al elemento de Streamlit */
h3,
[data-testid="stHeading"] h3,
[data-testid="stHeadingWithActionElements"] h3,
div[class*="stHeadingContainer"] h3 {
...
div[class*="stHeadingContainer"] h4 {
...
if st.session_state.area_activa is None:
    ...
    with col_centro:
        ...
        st.caption(
            "Contraseña por defecto: nombre del área en minúsculas "
            "sin espacios ni acentos. Ejemplo: urgenciaspediatricas"
        )

        st.divider()
        st.info(
            "Este sistema es una herramienta de apoyo clínico para la verificación "
            "de dosis de medicamentos pediátricos. No reemplaza el criterio del "
            "personal de salud ni la indicación del médico tratante."
        )
        ...
        with st.expander("¿Para qué sirve este sistema?"):
            st.markdown("""
El Sistema de Verificación de Dosis Segura es una herramienta de apoyo clínico diseñada para el personal de enfermería.

Su función principal es **verificar si la dosis de un medicamento pediátrico es segura antes de administrarla**, comparando la dosis preparada con el rango esperado según el peso del paciente.

El sistema también incorpora un componente de **Inteligencia Artificial** que clasifica el nivel de riesgo de cada caso, aprendiendo progresivamente con los datos registrados en cada área hospitalaria.
            """)

        with st.expander("¿Cómo inicio sesión?"):
            st.markdown("""
Selecciona el **área hospitalaria** donde te encuentras trabajando e ingresa la **contraseña** asignada. Presiona el botón **Ingresar**.

Cada área tiene su propio acceso, historial y medicamentos independientes.

La contraseña por defecto es el nombre del área en minúsculas, sin espacios ni acentos. Por ejemplo: **urgenciaspediatricas**

Si necesitas cambiar la contraseña, puedes hacerlo desde la pestaña **Configuración**.
            """)

        with st.expander("¿Cómo verifico una dosis?"):
            st.markdown("""
En la pestaña **Verificar dosis** ingresa la siguiente información:

**Datos del paciente:** número de expediente, edad, peso y diagnóstico clínico.

**Datos del medicamento:** nombre del medicamento y vía de administración.

**Datos de preparación:** concentración disponible en mg/mL y volumen preparado en mL.

Una vez completados todos los campos presiona el botón **Verificar dosis**.
            """)

        with st.expander("¿Cómo interpreto el resultado?"):
            st.markdown("""
El sistema mostrará una de estas tres clasificaciones:

**Dosis segura** — desviación menor al 10%. Puede administrarse.

**Advertencia** — desviación entre 10% y 20%. Revisa el cálculo antes de administrar.

**Alto riesgo** — desviación mayor al 20%. No administrar hasta verificar con el médico tratante.

La Inteligencia Artificial también emitirá su propia clasificación con un porcentaje de confianza.
            """)

        with st.expander("¿Qué hago si el medicamento no aparece en la lista?"):
            st.markdown("""
Ve a la pestaña **Medicamentos** e ingresa el nombre, descripción, dosis máxima y dosis recomendada por kilogramo según vía de administración.

Una vez registrado estará disponible únicamente para tu área. También puedes editar o eliminar medicamentos desde esa misma pestaña.
            """)

        with st.expander("¿Cómo funciona la Inteligencia Artificial?"):
            st.markdown("""
**Modo ya entrenado** — modelo entrenado con 500 casos clínicos simulados. Disponible desde el primer uso.

**Modo nuevo** — aprende con los casos reales registrados en tu área. Se activa automáticamente al acumular al menos 20 casos. Entre más casos se registren, más precisa será la clasificación.

Si usas el Modo ya entrenado con un medicamento no incluido en el entrenamiento original, el sistema te avisará que la clasificación puede no ser precisa.
            """)

        with st.expander("¿Cómo consulto el historial?"):
            st.markdown("""
En la pestaña **Historial** encontrarás todos los casos validados en tu área. Puedes filtrar por número de expediente, medicamento o clasificación, y descargar el registro en formato CSV.
            """)

        with st.expander("¿Qué muestra la Retroalimentación?"):
            st.markdown("""
Presenta un resumen estadístico: total de casos, distribución por clasificación, medicamentos con mayor frecuencia de alertas y tabla de errores frecuentes por medicamento.
            """)

        with st.expander("Aviso importante"):
            st.warning(
                "Este sistema es una herramienta de apoyo a la decisión clínica. "
                "No reemplaza el criterio del personal de salud ni la indicación "
                "del médico tratante. Ante cualquier duda, consulta siempre con el "
                "médico responsable antes de administrar el medicamento."
            )
...
# Header principal con banner institucional
...
with tab1:
    ...
    with col_m1:
        if st.button(
            "Modo ya entrenado — datos simulados",
            use_container_width=True,
            type="primary" if modo_ia == "simulado" else "secondary"
        ):
    ...
    with col_m2:
        if st.button(
            f"Modo nuevo — {n_casos} casos reales registrados",
            use_container_width=True,
            type="primary" if modo_ia == "real" else "secondary"
        ):
    ...
    if modo_ia == "simulado":
        ...
        if n_casos < MIN_CASOS_ENTRENAMIENTO:
            st.warning(
                f"Modo nuevo activo — {n_casos} de {MIN_CASOS_ENTRENAMIENTO} casos "
                f"reales requeridos. La clasificación por IA estará disponible al "
                f"alcanzar el mínimo."
            )
        else:
            st.success(
                f"Modo nuevo activo — modelo entrenado con {n_casos} casos reales."
            )
    ...
    with col_izq:
        ...
        peso = st.number_input(
            "Peso (kg)", min_value=0.1, max_value=150.0, step=0.1, format="%.1f"
        )
        ...
        st.caption(
            "Si el medicamento requerido no aparece en la lista, "
            "regístrelo en la pestaña Medicamentos antes de continuar."
        )
    ...
    with col_der:
        ...
        if dosis_sugerida > 0:
            st.info(
                f"Dosis recomendada para **{medicamento_sel}** "
                f"vía {via_corta}: **{dosis_sugerida} mg/kg**"
            )

        dosis_mgkg = st.number_input(
            "Dosis recomendada (mg/kg)",
            min_value=0.0,
            value=float(dosis_sugerida),
            step=0.1,
            format="%.2f"
        )
        dosis_prescrita = st.number_input(
            "Dosis prescrita (mg)", min_value=0.0, step=0.1, format="%.2f"
        )

        st.subheader("Preparación")
        concentracion = st.number_input(
            "Concentración disponible (mg/mL)",
            min_value=0.0, step=0.1, format="%.2f"
        )
        volumen = st.number_input(
            "Volumen preparado (mL)", min_value=0.0, step=0.1, format="%.2f"
        )

        if concentracion > 0 and volumen > 0:
            st.info(
                f"Dosis calculada de la preparación: **{concentracion * volumen:.2f} mg**"
            )
    ...
    if st.button("✅  Verificar dosis", use_container_width=True, type="primary"):
        if not no_expediente:
            ...
            with c2:
                st.metric("Dosis preparada", f"{dosis_preparada:.2f} mg",
                          delta=f"{dosis_preparada - dosis_esperada:+.2f} mg")
            with c3:
            ...
            if modelo:
                pred_ia, confianza_ia, _, med_conocido = predecir_riesgo(
                    modelo, le_med, le_via,
                    edad, peso, dosis_mgkg,
                    dosis_esperada, dosis_preparada,
                    medicamento_sel, via_sel
                )
                ...
                if not med_conocido:
                    st.warning(
                        "El medicamento seleccionado no forma parte del conjunto de "
                        "entrenamiento del modelo simulado. La clasificación puede no "
                        "ser precisa. Se recomienda activar el Modo nuevo una vez que "
                        "se acumulen casos reales con este medicamento."
                    )
            else:
                st.info(
                    f"La clasificación por IA estará disponible cuando se registren "
                    f"al menos {MIN_CASOS_ENTRENAMIENTO} casos reales. "
                    f"Casos actuales: {n_reales}."
                )
            ...
            st.caption(
                f"Caso registrado el {ahora.strftime('%d/%m/%Y')} a las "
                f"{ahora.strftime('%H:%M')} | Expediente: {no_expediente}"
            )
...
with tab2:
    ...
    st.caption(
        "Los medicamentos registrados aquí estarán disponibles "
        "únicamente para esta área."
    )
            ...
            with st.expander(f"💊  {nom} — {dat.get('descripcion', '')}"):
                with st.form(f"edit_{nom}"):
                    nueva_desc = st.text_input(
                        "Descripción", value=dat.get("descripcion", "")
                    )
                    nuevo_max = st.number_input(
                        "Dosis máxima (mg)",
                        min_value=0.0,
                        value=float(dat.get("dosis_max") or 0),
                        step=1.0
                    )
                    ...
                    with col_e1:
                        e_iv = st.number_input(
                            "Intravenosa (IV)", min_value=0.0,
                            value=float(dat["dosis_mgkg"].get("IV", 0)),
                            step=0.1, format="%.2f", key=f"iv_{nom}"
                        )
                    with col_e2:
                        e_oral = st.number_input(
                            "Oral", min_value=0.0,
                            value=float(dat["dosis_mgkg"].get("Oral", 0)),
                            step=0.1, format="%.2f", key=f"oral_{nom}"
                        )
                    with col_e3:
                        e_im = st.number_input(
                            "Intramuscular (IM)", min_value=0.0,
                            value=float(dat["dosis_mgkg"].get("IM", 0)),
                            step=0.1, format="%.2f", key=f"im_{nom}"
                        )
                    ...
                    with col_btn2:
                        eliminar = st.form_submit_button(
                            "Eliminar medicamento", use_container_width=True
                        )
    ...
    with st.form("form_nuevo_med"):
        ...
        submitted = st.form_submit_button(
            "Registrar medicamento", type="primary", use_container_width=True
        )
        ...
        if submitted:
            if not nombre:
                ...
                if not dosis_dict:
                    ...
                    guardar_medicamento_nuevo(
                        area, nombre, dosis_dict,
                        dosis_max if dosis_max > 0 else None,
                        descripcion
                    )
                    st.success(
                        f"Medicamento '{nombre}' registrado correctamente. "
                        f"Ya está disponible en la sección de verificación."
                    )
                    st.rerun()
...
with tab3:
    ...
    if df.empty:
        ...
        resumen = resumen[resumen["Casos con alerta"] > 0].sort_values(
            "Casos con alerta", ascending=False
        )
        if resumen.empty:
    ...
    if df.empty:
        ...
        with col_f2:
            filtro_med = st.multiselect(
                "Filtrar por medicamento", df["Medicamento"].unique()
            )
        with col_f3:
            filtro_clas = st.multiselect(
                "Filtrar por clasificación", df["Clasificacion"].unique()
            )

        df_filtrado = df.copy()
        if filtro_exp:
            df_filtrado = df_filtrado[
                df_filtrado["No_Expediente"].astype(str).str.contains(filtro_exp)
            ]
        if filtro_med:
...
with tab5:
    ...
    with st.form("form_password"):
        ...
        submitted_pwd = st.form_submit_button(
            "Actualizar contraseña", type="primary", use_container_width=True
        )
    ...
    if n_casos >= MIN_CASOS_ENTRENAMIENTO:
        ...
        st.warning(
            f"Se requieren al menos {MIN_CASOS_ENTRENAMIENTO} casos para activar "
            f"el modo nuevo. Casos faltantes: {MIN_CASOS_ENTRENAMIENTO - n_casos}."
        )

    st.divider()
    st.markdown("#### Reiniciar aprendizaje desde cero")
    st.warning(
        "Esta acción eliminará todo el historial real del área. "
        "El modelo volverá a empezar desde cero. "
